...
export async function withProductImages<T extends { id: number }>(rows: T[]): Promise<(T & { images: (typeof productImages.$inferSelect)[] })[]> {
  if (!rows.length) return [];
  const db = getDb();
  if (!db) return rows.map((r) => ({ ...r, images: [] }));
  const imgs = await db
    .select()
    .from(productImages)
    .where(inArray(productImages.productId, rows.map((r) => r.id)))
    .orderBy(productImages.sortOrder);
  const grouped = new Map<number, (typeof productImages.$inferSelect)[]>();
  for (const im of imgs) {
    const list = grouped.get(im.productId) ?? [];
    list.push(im);
    grouped.set(im.productId, list);
  }
...
export async function getProductsWithImages(limit = 100): Promise<ProductWithImages[]> {
  ...
  try {
    ...
    if (!rows.length) return demoProducts().slice(0, limit).map(demoToProduct);
...
export async function getRelatedProducts(categorySlug: string, excludeId: number, limit = 8): Promise<ProductWithImages[]> {
  ...
  try {
    ...
    return withProductImages(filtered);
  } catch {
    return demoRelated(categorySlug, excludeId, limit).map(demoToProduct);
  }
}
