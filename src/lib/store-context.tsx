...
export function StoreProvider({ children }: { children: React.ReactNode }) {
  ...
  const refreshUser = useCallback(async () => {
    try {
      ...
      setUser(j.user ?? null);
      if (j.user) {
        const local = readLS<number[]>("nyc_wishlist", []);
        const w = await fetch("/api/wishlist").then((x) => x.json()).catch(() => null);
        const serverIds: number[] = Array.isArray(w?.ids) ? w.ids : [];
        const missing = local.filter((id) => !serverIds.includes(id));
        if (missing.length && !w?.demo) {
          await Promise.all(missing.map((productId) => fetch("/api/wishlist", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ productId }),
          }).catch(() => null)));
        }
        setWishlist(Array.from(new Set([...local, ...serverIds])));
      }
    } catch {
      setUser(null);
    }
