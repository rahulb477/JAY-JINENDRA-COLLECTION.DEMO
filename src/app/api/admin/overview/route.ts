...
import { products, orders, users, offers, banners, reels, categories, siteSettings } from "@/db/schema";
import { desc, eq, count, sum } from "drizzle-orm";
import { verifySession, SESSION_COOKIE } from "@/lib/auth";
import { demoProducts, demoOffers, demoBanners, demoReels, demoCategories, demoSettings } from "@/lib/demo-catalog";

export const dynamic = "force-dynamic";

function isAdmin(req: NextRequest) {
  const t = req.cookies.get(SESSION_COOKIE)?.value;
  const u = t ? verifySession(t) : null;
  const key = req.nextUrl.searchParams.get("key") ?? req.headers.get("x-admin-key");
  const ADMIN_KEY = process.env.NYC_ADMIN_KEY || "jjc-admin-2026";
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const settings = { ...demoSettings(), ...Object.fromEntries(settingRows.map((r) => [r.key, r.value])) };
    return NextResponse.json({
      stats: {
        products: Number(pc.c), orders: Number(oc.c), users: Number(uc.c),
        revenue: Number(rev[0]?.s ?? 0),
      },
      recentOrders, allProducts, allOffers, allBanners, allReels, allCats, settings,
