...
import { deliveryChargeFor, freeShippingAbove, useCommerceSettings } from "@/lib/use-settings";
...
export default function CheckoutPage() {
  ...
  const [savedAddresses, setSavedAddresses] = useState<{ id: number; label: string | null; fullName: string; mobile: string; house: string; area: string; city: string; state: string; pincode: string }[]>([]);

  useEffect(() => {
    if (!user) return;
    setName((n) => n || user.name);
    setEmail((e) => e || user.email);
    fetch("/api/addresses").then((r) => r.json()).then((j) => {
      const list = j.addresses ?? [];
      setSavedAddresses(list);
      const a = list[0];
      if (!a) return;
      setName((n) => n || a.fullName);
      setMobile((m) => m || a.mobile);
      setHouse((h) => h || a.house);
      setArea((ar) => ar || a.area);
      setCity(a.city || "Jodhpur");
      setState(a.state || "Rajasthan");
      setPincode((p) => p || a.pincode);
    }).catch(() => {});
  }, [user]);

  const savings = cartMrp - cartSubtotal;
  const freeAbove = freeShippingAbove(settings);
  const deliveryCharge = useMemo(() => deliveryChargeFor(cartSubtotal, delivery, settings), [delivery, cartSubtotal, settings]);
  ...
  const placeOrder = async () => {
    ...
    try {
      ...
      saveLocalOrder({
        ...
        items: cart.map((c) => ({ name: c.name, image: c.image, size: c.size, qty: c.qty, price: c.price })),
      });
      const alreadySaved = savedAddresses.some((a) => a.house === house && a.pincode === pincode);
      if (user && saveAddress && delivery === "HOME_DELIVERY" && !alreadySaved) {
        fetch("/api/addresses", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ fullName: name, mobile, house, area, city, state, pincode, label: "Home" }),
        }).catch(() => {});
      }
  ...
  if (step === "success") {
    return (
      <div className="max-w-xl mx-auto px-4 py-14 text-center">
        ...
        <p className="text-sm text-neutral-500 mt-2">Thank you for shopping with Jai Jinendra Collection.</p>
        <div className="mt-5 rounded-2xl border-2 border-dashed border-[#c9a24b] bg-[#f5f5f5] p-5">
        ...
        <div className="grid grid-cols-2 gap-2 mt-6">
          <Link href="/account" className="py-3.5 rounded-xl bg-black text-[#e8c877] font-extrabold text-[12.5px] tracking-wider">TRACK ORDER</Link>
        ...
        <a href={`https://wa.me/919468623457?text=${encodeURIComponent(`Hi! I just placed order ${orderNumber} on your website.`)}`} target="_blank" rel="noreferrer" className="mt-2.5 flex items-center justify-center gap-2 py-3.5 rounded-xl bg-[#25D366] text-white font-extrabold text-[12.5px]">
          CONFIRM ON WHATSAPP
        </a>
      </div>
    );
  ...
  if (cart.length === 0) {
    return (
      <div className="max-w-xl mx-auto px-4 py-16 text-center">
        ...
        <Link href="/shop" className="inline-block mt-6 px-8 py-3.5 rounded-xl bg-black text-[#e8c877] font-extrabold text-[13px]">SHOP COLLECTION</Link>
      </div>
    );
  }
    ...
    <div className="max-w-[1200px] mx-auto px-4 sm:px-6 py-6 sm:py-10">
      ...
      <div className="grid lg:grid-cols-[1fr_400px] gap-6 mt-6 items-start">
        <div className="space-y-5">
          ...
          <section className="border border-neutral-200 rounded-3xl p-5 sm:p-6">
            ...
            <div className="grid sm:grid-cols-2 gap-3 mt-4">
              <div className="sm:col-span-1"><label className="text-[12px] font-bold text-neutral-600">Full name *</label><input value={name} onChange={(e) => setName(e.target.value)} placeholder="e.g. Rahul Sharma" className={`${input} mt-1`} /></div>
          ...
          <section className="border border-neutral-200 rounded-3xl p-5 sm:p-6">
            ...
            <div className="grid sm:grid-cols-2 gap-3 mt-4">
              <button onClick={() => setDelivery("HOME_DELIVERY")} className={`flex items-start gap-3 p-4 rounded-2xl border-2 text-left transition ${delivery === "HOME_DELIVERY" ? "border-black bg-neutral-900 text-white" : "border-neutral-200"}`}>
                <Truck size={22} className={delivery === "HOME_DELIVERY" ? "text-[#e8c877]" : "text-neutral-400"} />
                <span><span className="block font-extrabold text-[14px]">HOME DELIVERY</span><span className={`text-[12px] ${delivery === "HOME_DELIVERY" ? "text-neutral-300" : "text-neutral-500"}`}>2–5 days · {cartSubtotal >= freeAbove ? "FREE" : formatINR(Number(settings.delivery_charge) || 49)}</span></span>
              </button>
              <button onClick={() => setDelivery("STORE_PICKUP")} className={`flex items-start gap-3 p-4 rounded-2xl border-2 text-left transition ${delivery === "STORE_PICKUP" ? "border-black bg-neutral-900 text-white" : "border-neutral-200"}`}>
                <StoreIcon size={22} className={delivery === "STORE_PICKUP" ? "text-[#e8c877]" : "text-neutral-400"} />
                <span><span className="block font-extrabold text-[14px]">STORE PICKUP · FREE</span><span className={`text-[12px] ${delivery === "STORE_PICKUP" ? "text-neutral-300" : "text-neutral-500"}`}>Same-day · Sarafa Bazaar</span></span>
              </button>
            </div>
            {delivery === "HOME_DELIVERY" ? (
              <div className="grid sm:grid-cols-2 gap-3 mt-4">
                {savedAddresses.length > 0 && (
                  <div className="sm:col-span-2 flex gap-2 overflow-x-auto no-scrollbar pb-1">
                    {savedAddresses.map((a) => (
                      <button key={a.id} type="button" onClick={() => { setName(a.fullName); setMobile(a.mobile); setHouse(a.house); setArea(a.area); setCity(a.city); setState(a.state); setPincode(a.pincode); }} className="shrink-0 px-3 py-2 rounded-full border border-neutral-200 text-[12px] font-bold hover:border-black">
                        {a.label || "Saved"} · {a.area}
                      </button>
                    ))}
                  </div>
                )}
                ...
                {user && <label className="sm:col-span-2 flex items-center gap-2 text-[12.5px] font-semibold"><input type="checkbox" checked={saveAddress} onChange={(e) => setSaveAddress(e.target.checked)} /> Save this address to my account</label>}
              </div>
            ) : (
              <div className="mt-4 rounded-2xl bg-[#f5f5f5] border border-[#e4e4e4] p-4 text-[13px] text-neutral-700">
                <p className="font-extrabold">Pick up from our store — FREE</p>
                <p className="mt-1">{STORE.address1}, {STORE.address2}, {STORE.city}</p>
                <p className="text-neutral-500 mt-0.5">Open {settings.store_hours || STORE.hours} · We&apos;ll call {mobile || "you"} when your order is ready.</p>
              </div>
            )}
          </section>
            ...
            <div className="space-y-2.5 mt-4">
              {[
                ...
                <button key={m.id} disabled={!m.enabled} onClick={() => setPayment(m.id)} className={`w-full flex items-center gap-3 p-4 rounded-2xl border-2 text-left transition ${payment === m.id ? "border-black bg-neutral-900 text-white" : "border-neutral-200"} ${!m.enabled ? "opacity-50" : ""}`}>
                  <span className={payment === m.id ? "text-[#e8c877]" : "text-neutral-400"}>{m.icon}</span>
                  ...
                  <span className={`w-5 h-5 rounded-full border-2 grid place-items-center ${payment === m.id ? "border-[#e8c877]" : "border-neutral-300"}`}>{payment === m.id && <span className="w-2.5 h-2.5 rounded-full bg-[#e8c877]" />}</span>
                </button>
              ))}
            </div>
        ...
        <div className="border border-neutral-200 rounded-3xl p-5 sm:p-6 lg:sticky lg:top-32">
          ...
          <button onClick={placeOrder} disabled={placing} className="mt-5 w-full py-4 rounded-xl bg-black text-[#e8c877] font-extrabold text-[13px] tracking-[0.12em] disabled:opacity-60">
