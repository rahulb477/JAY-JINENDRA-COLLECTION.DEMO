...
export default function AdminPage() {
  ...
  const load = async (k: string) => {
    ...
    try {
      ...
      setMsg("Could not load admin data. Seed the database first.");
    } finally {
      setLoading(false);
    }
  };
    ...
    return (
      <div className="max-w-md mx-auto px-4 py-16">
        <div className="border border-neutral-200 rounded-3xl p-8 text-center">
          <span className="inline-grid place-items-center w-14 h-14 rounded-full bg-black text-[#e8c877]"><Lock size={24} /></span>
          <h1 className="font-display font-extrabold text-2xl mt-4">Admin Panel</h1>
          <p className="text-[13px] text-neutral-500 mt-1">Enter admin key to manage Jai Jinendra Collection.<br />(Default: <code className="bg-neutral-100 px-1.5 py-0.5 rounded">jjc-admin-2026</code>)</p>
          <p className="text-[12px] text-neutral-500 mt-2">Staff login after seeding: <b>admin@jaijinendracollection.com</b> / same key as password.</p>
          ...
          <button onClick={() => { if (key) { localStorage.setItem("nyc_admin_key", key); setAuthed(true); } }} className="mt-3 w-full py-3.5 rounded-xl bg-black text-[#e8c877] font-extrabold text-[13px]">UNLOCK PANEL</button>
  ...
  return (
    <div className="max-w-[1300px] mx-auto px-4 sm:px-6 py-6 sm:py-8">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <p className="text-[11px] font-bold tracking-[0.25em] text-[#9a7420]">JAI JINENDRA COLLECTION</p>
              ...
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
                {[
                  ...
                  <div key={s.l} className="rounded-2xl border border-neutral-200 p-5 bg-white">
                    <span className="inline-grid place-items-center w-10 h-10 rounded-xl bg-black text-[#e8c877]">{s.icon}</span>
  ...
  return (
    <div className="border border-neutral-200 rounded-2xl overflow-hidden overflow-x-auto">
      <table className="w-full text-[13px] min-w-[720px]">
        <thead><tr className="bg-neutral-900 text-[#e8c877] text-left"><th className="p-3">Order</th><th className="p-3">Customer</th><th className="p-3">Total</th><th className="p-3">Type</th><th className="p-3">Status</th><th className="p-3">Date</th></tr></thead>
        <tbody>
          {orders.map((o) => (
  ...
  return (
    <div>
      <div className="flex gap-2 mb-4">
        ...
        <button onClick={() => setShowAdd(!showAdd)} className="px-5 py-2.5 rounded-xl bg-black text-[#e8c877] text-[12px] font-extrabold flex items-center gap-1.5"><Plus size={14} /> ADD</button>
      </div>
      {showAdd && (
        <div className="border-2 border-dashed border-[#c9a24b] rounded-2xl p-4 mb-4 grid sm:grid-cols-3 gap-2.5 bg-[#f5f5f5]">
          ...
          <button onClick={() => { onAct("create-product", { ...add, audience: add.categorySlug.startsWith("kids") ? "kids" : "men" }); setShowAdd(false); }} className="sm:col-span-3 py-2.5 rounded-xl brand-bg text-black text-[12px] font-extrabold">CREATE PRODUCT</button>
        </div>
      )}
      <div className="border border-neutral-200 rounded-2xl overflow-hidden overflow-x-auto">
        <table className="w-full text-[13px] min-w-[760px]">
          <thead><tr className="bg-neutral-900 text-[#e8c877] text-left"><th className="p-3">Product</th><th className="p-3">Price</th><th className="p-3">Stock</th><th className="p-3">Flags</th><th className="p-3">Active</th><th className="p-3">Actions</th></tr></thead>
          <tbody>
            {list.map((p) => (
              <tr key={p.id} className="border-t border-neutral-100">
                ...
                <td className="p-3 text-[11px]">
                  {editing === p.id ? (
                    ...
                    <span className="flex gap-1">{p.isNewArrival && <span className="bg-emerald-100 text-emerald-800 px-1.5 py-0.5 rounded font-bold">NEW</span>}{p.isTrending && <span className="bg-black text-[#e8c877] px-1.5 py-0.5 rounded font-bold">TREND</span>}</span>
                  )}
                </td>
                <td className="p-3">
                  {editing === p.id ? (
...
function AddOfferForm({ onAdd }: { onAdd: (f: Record<string, string>) => void }) {
  ...
  if (!open) return <button onClick={() => setOpen(true)} className="px-5 py-2.5 rounded-xl bg-black text-[#e8c877] text-[12px] font-extrabold flex items-center gap-1.5"><Plus size={14} /> NEW OFFER</button>;
  const inp = "border border-neutral-200 rounded-lg px-3 py-2 text-[13px] w-full bg-white";
  return (
    <div className="border-2 border-dashed border-[#c9a24b] rounded-2xl p-4 grid sm:grid-cols-3 gap-2 bg-[#f5f5f5]">
      ...
      <button onClick={() => { onAdd(f); setOpen(false); }} className="sm:col-span-3 py-2.5 rounded-xl brand-bg text-black text-[12px] font-extrabold">ADD OFFER</button>
    </div>
  );
}
...
function AddBannerForm({ onAdd }: { onAdd: (f: Record<string, string>) => void }) {
  ...
  const [open, setOpen] = useState(false);
  if (!open) return <button onClick={() => setOpen(true)} className="px-5 py-2.5 rounded-xl bg-black text-[#e8c877] text-[12px] font-extrabold flex items-center gap-1.5"><Plus size={14} /> NEW BANNER</button>;
  const inp = "border border-neutral-200 rounded-lg px-3 py-2 text-[13px] w-full bg-white";
  return (
    <div className="border-2 border-dashed border-[#c9a24b] rounded-2xl p-4 grid sm:grid-cols-2 gap-2 bg-[#f5f5f5]">
      <input value={f.title} onChange={(e) => setF({ ...f, title: e.target.value })} placeholder="Title" className={inp} />
      <input value={f.subtitle} onChange={(e) => setF({ ...f, subtitle: e.target.value })} placeholder="Subtitle" className={inp} />
      ...
      <button onClick={() => { onAdd(f); setOpen(false); }} className="sm:col-span-2 py-2.5 rounded-xl brand-bg text-black text-[12px] font-extrabold">ADD BANNER</button>
    </div>
  );
}
...
function AddReelForm({ onAdd }: { onAdd: (f: Record<string, string>) => void }) {
  ...
  const [open, setOpen] = useState(false);
  if (!open) return <button onClick={() => setOpen(true)} className="px-5 py-2.5 rounded-xl bg-black text-[#e8c877] text-[12px] font-extrabold flex items-center gap-1.5"><Plus size={14} /> ADD REEL</button>;
  const inp = "border border-neutral-200 rounded-lg px-3 py-2 text-[13px] w-full bg-white";
  return (
    <div className="border-2 border-dashed border-[#c9a24b] rounded-2xl p-4 grid sm:grid-cols-3 gap-2 bg-[#f5f5f5]">
      ...
      <button onClick={() => { onAdd(f); setOpen(false); }} className="sm:col-span-3 py-2.5 rounded-xl brand-bg text-black text-[12px] font-extrabold">ADD REEL</button>
    </div>
  );
}

function SettingsForm({ initial, onSave }: { initial?: Record<string, string>; onSave: (s: Record<string, string>) => void }) {
  const blank = { announcement: "FREE SHIPPING ON SELECTED ORDERS • EASY STORE PICKUP", delivery_charge: "49", free_shipping_above: "1499", store_phone: "+91 94686 23457", store_address: "Sarafa Bazaar, Jodhpur, Rajasthan, India", instagram_url: "https://instagram.com/jai_jinendra_collection__" };
  const [s, setS] = useState(blank);
  useEffect(() => {
    if (!initial) return;
    setS((prev) => ({ ...prev, ...Object.fromEntries(Object.keys(blank).map((k) => [k, initial[k] ?? prev[k as keyof typeof prev]])) }));
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [initial]);
  ...
  return (
    <div className="max-w-2xl border border-neutral-200 rounded-2xl p-5 space-y-3">
      ...
      <button onClick={() => onSave(s)} className="w-full py-3 rounded-xl bg-black text-[#e8c877] text-[13px] font-extrabold">SAVE SETTINGS</button>
    </div>
  );
}
