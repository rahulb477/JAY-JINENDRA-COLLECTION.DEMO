...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  try {
    ...
    return {
      ...
      openGraph: { title: `${p.name} | Jai Jinendra Collection`, description: p.shortDesc ?? "", images: p.images[0]?.url ? [p.images[0].url] : [] },
    };
  } catch {
...
export default async function ProductPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const jsonLd = {
    ...
    brand: { "@type": "Brand", name: "Jai Jinendra Collection" },
