...
export default function WishlistPage() {
  ...
  useEffect(() => {
    if (!wishlist.length) {
      const empty = setTimeout(() => { setProducts([]); setLoading(false); }, 0);
  ...
  return (
    <div className="max-w-[1400px] mx-auto px-4 sm:px-6 py-6 sm:py-10">
      <h1 className="font-display font-extrabold text-2xl sm:text-3xl flex items-center gap-2.5">
        <Heart size={26} className="fill-[#c9a24b] text-[#c9a24b]" /> Wishlist
      </h1>
      <p className="text-sm text-neutral-500 mt-1">
        {user ? `Saved to your account (${wishlist.length})` : "Saved on this device — login to sync across devices."}
        {!user && <Link href="/auth" className="ml-2 font-bold text-[#9a7420] underline">Login</Link>}
      </p>
      {loading ? (
        ...
        <div className="text-center py-16">
          ...
          <Link href="/trending" className="inline-block mt-5 px-8 py-3.5 rounded-xl bg-black text-[#e8c877] font-extrabold text-[13px]">EXPLORE TRENDING</Link>
        </div>
      ) : (
