...
function AuthForm() {
  ...
  return (
    <div className="min-h-[70vh] grid lg:grid-cols-2 max-w-5xl mx-4 sm:mx-6 lg:mx-auto my-6 sm:my-10 rounded-3xl overflow-hidden border border-neutral-200">
      <div className="hidden lg:flex flex-col justify-between bg-black p-10 hero-grain relative">
        <Logo light />
        <div>
          <h2 className="font-display font-black text-white text-4xl leading-tight">Join the<br /><span className="text-[#c9a24b]">JJC family.</span></h2>
          <p className="text-[11px] font-bold tracking-[0.3em] text-neutral-400 mt-3">WHOLESALE • RETAIL · DELIVERY ACROSS INDIA</p>
          <p className="text-neutral-400 text-sm mt-3 max-w-xs">Track orders, sync wishlist, faster checkout & early access to drops.</p>
        </div>
        <p className="text-neutral-500 text-xs">Sarafa Bazaar · Jodhpur · Since style began</p>
      </div>
      <div className="p-6 sm:p-10 bg-white">
        <div className="lg:hidden mb-6"><Logo /></div>
        <div className="grid grid-cols-2 bg-neutral-100 rounded-xl p-1 mb-6">
          {(["login", "signup"] as const).map((m) => (
            <button key={m} onClick={() => { setMode(m); setError(""); }} className={`py-2.5 rounded-lg text-[13px] font-extrabold tracking-wider transition ${mode === m ? "bg-black text-[#e8c877]" : "text-neutral-500"}`}>
        ...
        <button onClick={submit} disabled={busy} className="mt-5 w-full py-4 rounded-xl bg-black text-[#e8c877] font-extrabold text-[13px] tracking-[0.12em] disabled:opacity-60">
          {busy ? "PLEASE WAIT…" : mode === "login" ? "LOGIN" : "CREATE ACCOUNT"}
        </button>
        <p className="text-center text-[12px] text-neutral-500 mt-4">
          {mode === "login" ? "New here? " : "Already have an account? "}
          <button onClick={() => setMode(mode === "login" ? "signup" : "login")} className="font-bold text-[#9a7420]">{mode === "login" ? "Create account" : "Login"}</button>
        </p>
      </div>
    </div>
  );
