...
export default function CartPage() {
  ...
  const delivery = deliveryChargeFor(cartSubtotal, "HOME_DELIVERY", settings);
  ...
  if (cart.length === 0) {
    return (
      <div className="max-w-2xl mx-auto px-4 py-16 text-center">
        ...
        <p className="text-neutral-500 text-sm mt-2">Discover trending styles from Jai Jinendra Collection.</p>
        <div className="flex gap-2 justify-center mt-6">
          <Link href="/shop" className="px-8 py-3.5 rounded-xl bg-black text-[#e8c877] font-extrabold text-[13px] tracking-wider">SHOP COLLECTION</Link>
          <Link href="/offers" className="px-8 py-3.5 rounded-xl brand-bg text-black font-extrabold text-[13px] tracking-wider">VIEW OFFERS</Link>
        </div>
              ...
              {saveForLater.map((item) => (
                <div key={item.key} className="flex gap-3 p-3 rounded-2xl border border-neutral-200">
                  ...
                  <div className="flex-1">
                    ...
                    <button onClick={() => moveToCart(item.key)} className="mt-1.5 text-[12px] font-extrabold text-[#9a7420]">MOVE TO BAG</button>
                  </div>
                </div>
              ))}
            </div>
      ...
      <div className="grid lg:grid-cols-[1fr_380px] gap-6 mt-6 items-start">
          ...
          {cart.map((item) => (
            <div key={item.key} className="flex gap-3 sm:gap-4 p-3 sm:p-4 rounded-2xl border border-neutral-200">
              ...
              <div className="flex-1 min-w-0">
                <Link href={`/product/${item.slug}`} className="font-bold text-[14px] sm:text-[15px] leading-snug hover:text-[#9a7420]">{item.name}</Link>
                ...
                {saveForLater.map((item) => (
                  <div key={item.key} className="flex gap-3 p-3 rounded-2xl border border-dashed border-neutral-300">
                    ...
                    <div className="min-w-0">
                      ...
                      <button onClick={() => moveToCart(item.key)} className="text-[11px] font-extrabold text-[#9a7420]">MOVE TO BAG</button>
                    </div>
                  </div>
                ))}
              </div>
        ...
        <div className="border border-neutral-200 rounded-3xl p-5 sm:p-6 lg:sticky lg:top-32 bg-white">
          ...
          <div className="space-y-2.5 mt-4 text-[14px]">
            ...
            {delivery > 0 && <p className="text-[12px] text-neutral-500">Add {formatINR(Math.max(0, freeAbove - cartSubtotal))} more for FREE delivery</p>}
          ...
          <Link href="/checkout" className="mt-5 flex items-center justify-center gap-2 w-full py-4 rounded-xl bg-black text-[#e8c877] font-extrabold text-[13px] tracking-[0.12em]">
