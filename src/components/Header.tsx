...
import { formatINR, cn, STORE } from "@/lib/utils";
...
export default function Header() {
  ...
  useEffect(() => {
    if (!q.trim() || !searchOpen) {
      const clear = setTimeout(() => setLive([]), 0);
  ...
  return (
    <>
      ...
      <div className="bg-black text-center py-2 px-3 relative z-[60]">
        <p className="text-[10.5px] sm:text-[11.5px] font-semibold tracking-[0.12em] sm:tracking-[0.18em] text-[#e8c877] leading-snug">
          {settings.announcement || "FREE SHIPPING ON SELECTED ORDERS • EASY STORE PICKUP"}
        </p>
      </div>
      ...
      <header className={cn("sticky top-0 z-50 bg-white/95 backdrop-blur-md border-b transition-shadow", stuck ? "shadow-[0_8px_30px_-12px_rgba(0,0,0,0.25)] border-neutral-200" : "border-neutral-100")}>
          ...
          <div className="flex items-center gap-2 sm:gap-6 h-[62px] sm:h-[72px]">
            ...
            <nav className="hidden lg:flex items-center gap-7 ml-4" aria-label="Primary">
              {NAV.map((n) => (
                <Link
                  ...
                  href={n.href}
                  className={cn(
                    "relative text-[13px] font-bold tracking-[0.1em] py-2 transition hover:text-[#9a7420]",
                    n.highlight ? "text-[#9a7420]" : "text-neutral-900"
                  )}
                >
                  {n.label}
            ...
            <div className="flex items-center gap-0.5 sm:gap-1">
              ...
              <Link href="/wishlist" className="w-10 h-10 grid place-items-center relative" aria-label="Wishlist">
                <Heart size={21} />
                {wishlist.length > 0 && (
                  <span className="absolute top-0.5 right-0 min-w-[18px] h-[18px] px-1 rounded-full bg-black text-[#e8c877] text-[10px] font-bold grid place-items-center">
                    {wishlist.length}
                  </span>
                )}
              </Link>
              <button onClick={() => setCartOpen(true)} className="w-10 h-10 grid place-items-center relative" aria-label="Cart">
                <ShoppingBag size={21} />
                {cartCount > 0 && (
                  <span className="absolute top-0.5 right-0 min-w-[18px] h-[18px] px-1 rounded-full bg-[#c9a24b] text-black text-[10px] font-extrabold grid place-items-center">
                    {cartCount}
                  </span>
                )}
              </button>
        ...
        <nav className="lg:hidden border-t border-neutral-100 overflow-x-auto no-scrollbar" aria-label="Categories">
          <div className="flex gap-6 px-4 py-2.5 w-max">
            {NAV.map((n) => (
              <Link key={n.label} href={n.href} className={cn("text-[12px] font-bold tracking-wider whitespace-nowrap", n.highlight ? "text-[#9a7420]" : "text-neutral-800")}>
                {n.label}
              </Link>
            ))}
                ...
                {live.length > 0 ? (
                    ...
                    {live.map((p) => (
                      <button key={p.slug} onClick={() => { setSearchOpen(false); router.push(`/product/${p.slug}`); }} className="w-full flex items-center gap-3 p-3 hover:bg-neutral-50 text-left">
                        ...
                        <span className="flex-1 min-w-0">
                          ...
                          <span className="text-sm font-bold text-[#9a7420]">{formatINR(p.price)}</span>
                        </span>
                        <ArrowRight size={18} className="text-neutral-300" />
                      </button>
                    ))}
                    <button onClick={() => doSearch(q)} className="w-full p-3.5 bg-black text-[#e8c877] text-sm font-bold tracking-wide">
                  ...
                  <div className="mt-6">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {SUGGESTIONS.map((s) => (
                        <button key={s} onClick={() => doSearch(s)} className="px-4 py-2 rounded-full border border-neutral-200 text-sm font-medium hover:border-[#c9a24b] hover:text-[#9a7420] transition">
                          {s}
                        </button>
                      ))}
                    </div>
              ...
              <div className="flex-1 overflow-y-auto p-4">
                {[
                  ...
                  <Link key={l.label} href={l.href} onClick={() => setMenuOpen(false)} className="flex items-center justify-between py-4 border-b border-neutral-100 group">
                    <span>
                      <span className={cn("block font-extrabold tracking-wide text-[15px]", l.hot ? "text-[#9a7420]" : "")}>{l.label} {l.hot && <span className="ml-1 text-[9px] bg-red-600 text-white px-1.5 py-0.5 rounded">HOT</span>}</span>
                ...
                <div className="grid grid-cols-2 gap-2 mt-4">
                  ...
                  <a href="tel:+919468623457" className="flex items-center gap-2 p-3 rounded-xl bg-neutral-100 text-sm font-semibold"><Phone size={16} /> Call Us</a>
                </div>
              </div>
              <div className="p-4 border-t border-neutral-100">
                <a href={`https://wa.me/${(settings.store_whatsapp || STORE.phoneRaw).replace(/\D/g, "")}?text=Hi%20Jai%20Jinendra%20Collection%2C%20I%20want%20to%20know%20more%20about%20your%20collection.`} target="_blank" rel="noreferrer" className="flex items-center justify-center gap-2 w-full py-3.5 rounded-xl bg-[#25D366] text-white font-bold text-sm">
                  WHATSAPP US
                </a>
              </div>
            </motion.aside>
              ...
              {cart.length === 0 ? (
                <div className="flex-1 grid place-items-center p-8 text-center">
                  <div>
                    ...
                    <button onClick={() => { setCartOpen(false); router.push("/men"); }} className="px-8 py-3 rounded-xl bg-black text-[#e8c877] font-bold text-sm tracking-wide">
                      START SHOPPING
                    </button>
                  </div>
                </div>
                ...
                <>
                  ...
                  <div className="p-4 border-t border-neutral-100 bg-neutral-50/60">
                    ...
                    <div className="flex justify-between text-sm mb-3"><span className="text-neutral-500">Delivery</span><span className="font-bold text-emerald-700">{deliveryChargeFor(cartSubtotal, "HOME_DELIVERY", settings) === 0 ? "FREE" : "Calculated at checkout"}</span></div>
                    <div className="grid grid-cols-2 gap-2">
                      ...
                      <button onClick={() => { setCartOpen(false); router.push("/checkout"); }} className="py-3.5 rounded-xl bg-black text-[#e8c877] font-bold text-[13px] tracking-wide">CHECKOUT</button>
                    </div>
                  </div>
                </>
              )}
          ...
          {[
            ...
            t.action ? (
              ...
              <Link key={t.label} href={t.href} className="flex flex-col items-center justify-center gap-0.5 text-neutral-700 relative">
                ...
                {!!t.count && <span className="absolute top-1 right-1/2 translate-x-5 min-w-[16px] h-4 px-1 rounded-full bg-[#c9a24b] text-black text-[9px] font-extrabold grid place-items-center">{t.count}</span>}
              </Link>
            )
          )}
        </div>
