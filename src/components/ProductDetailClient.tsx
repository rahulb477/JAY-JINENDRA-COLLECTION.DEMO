...
import { freeShippingAbove, useCommerceSettings } from "@/lib/use-settings";
...
export default function ProductDetailClient({ product, related, completeLook }: { product: P; related: CardProduct[]; completeLook: CardProduct[] }) {
  ...
  const freeAbove = freeShippingAbove(settings);
  const inStock = (product.stockTotal ?? 1) > 0;
  ...
  useEffect(() => {
    ...
    try {
      ...
      const next = [{ slug: product.slug, name: product.name, image: product.images[0]?.url ?? "", price: product.price }, ...raw.filter((r) => r.slug !== product.slug)].slice(0, 10);
      localStorage.setItem("nyc_recent", JSON.stringify(next));
    } catch {}
  }, [product.id, product.slug, product.name, product.images, product.price]);
  ...
  return (
    <div className="max-w-[1400px] mx-auto px-4 sm:px-6 py-5 sm:py-8 pb-28 lg:pb-8">
      ...
      <div className="grid lg:grid-cols-2 gap-6 lg:gap-12">
        ...
        <div>
          <div className="relative aspect-[3/4] rounded-3xl overflow-hidden bg-neutral-100">
            {product.images[imgIdx]?.url ? (
              ...
              <div className="absolute inset-0 grid place-items-center bg-neutral-900 text-[#e8c877] font-display">{product.name}</div>
            )}
            {pct > 0 && <span className="absolute top-4 left-4 text-xs font-extrabold px-3 py-1.5 rounded-full bg-[#c9a24b] text-black">{pct}% OFF</span>}
            <div className="absolute top-4 right-4 flex flex-col gap-1.5">
              {(product.badges ?? []).map((b) => (
                <span key={b} className="text-[10px] font-extrabold px-2.5 py-1 rounded-full bg-black/85 text-[#e8c877] border border-[#c9a24b]/50">{b}</span>
              ))}
            </div>
          </div>
        ...
        {/* Info */}
        <div>
          <p className="text-[11px] font-bold tracking-[0.22em] text-[#9a7420] uppercase">{product.categoryName ?? product.categorySlug.replace(/-/g, " ")} · JJC</p>
          ...
          {/* Offers strip */}
          <div className="mt-4 rounded-2xl border border-[#e4e4e4] bg-[#f5f5f5] p-3.5 space-y-1.5 text-[12.5px]">
            <p className="font-bold text-[12px] tracking-wider text-[#9a7420]">AVAILABLE OFFERS</p>
            <p>• Combo deals starting ₹999 — mix & match in store</p>
            <p>• Free shipping on orders above {formatINR(freeAbove)}</p>
            <p>• Easy store pickup at Sarafa Bazaar, Jodhpur</p>
          </div>
            ...
            <div className="mt-5">
              ...
              <div className="flex gap-2 mt-2">
                {product.colors.map((c) => (
                  <button key={c} onClick={() => setColor(c)} className={cn("px-4 h-10 rounded-lg border text-[13px] font-bold", color === c ? "border-black bg-black text-[#e8c877]" : "border-neutral-200")}>{c}</button>
                ))}
              </div>
            </div>
          )}
          ...
          <div className="mt-5">
            <div className="flex items-center justify-between">
              ...
              <button onClick={() => setSizeGuide(true)} className="flex items-center gap-1 text-[12px] font-bold text-[#9a7420] underline underline-offset-2"><Ruler size={14} /> SIZE GUIDE</button>
            </div>
            <div className="flex flex-wrap gap-2 mt-2">
              {(product.sizes?.length ? product.sizes : ["S", "M", "L", "XL", "XXL"]).map((s) => (
                <button key={s} onClick={() => { setSize(s); setSizeError(false); }} className={cn("min-w-[52px] h-11 px-3 rounded-xl border-2 text-sm font-extrabold transition", size === s ? "border-black bg-black text-[#e8c877]" : sizeError ? "border-red-300" : "border-neutral-200 hover:border-black")}>{s}</button>
              ))}
            </div>
          </div>
          ...
          <div className="mt-5 flex items-center gap-3">
            ...
            <span className={`text-[12px] font-bold flex items-center gap-1 ${inStock ? "text-emerald-700" : "text-red-600"}`}><span className={`w-2 h-2 rounded-full ${inStock ? "bg-emerald-500 animate-pulse" : "bg-red-500"}`} /> {inStock ? "In stock" : "Out of stock — WhatsApp us"}</span>
          </div>

          {/* CTAs */}
          <div className="hidden lg:flex gap-2.5 mt-6">
            <button onClick={() => toggleWishlist(product.id)} className={cn("w-[52px] h-[52px] grid place-items-center rounded-xl border-2 shrink-0", wished ? "border-black bg-black text-[#e8c877]" : "border-neutral-200")} aria-label="Wishlist">
              <Heart size={20} className={wished ? "fill-[#e8c877]" : ""} />
            </button>
            <button onClick={handleAdd} disabled={!inStock} className="flex-1 h-[52px] rounded-xl border-2 border-black font-extrabold text-[13px] tracking-wider flex items-center justify-center gap-2 hover:bg-neutral-100 disabled:opacity-40"><ShoppingBag size={18} /> ADD TO CART</button>
            <button onClick={handleBuy} disabled={!inStock} className="flex-1 h-[52px] rounded-xl bg-black text-[#e8c877] font-extrabold text-[13px] tracking-wider flex items-center justify-center gap-2 hover:bg-neutral-900 disabled:opacity-40"><Zap size={18} /> BUY NOW</button>
          </div>
          <a href={whatsappEnquiry(product.name, product.price, size)} target="_blank" rel="noreferrer" className="hidden lg:flex mt-2.5 h-[52px] rounded-xl bg-[#25D366] text-white font-extrabold text-[13px] tracking-wider items-center justify-center gap-2">
          ...
          <div className="mt-5 grid grid-cols-3 gap-2 text-center">
            {[
              ...
              <div key={x.t} className="rounded-xl bg-neutral-50 border border-neutral-100 p-3">
                <span className="inline-grid place-items-center text-[#9a7420]">{x.icon}</span>
          ...
          <div className="mt-5 border-t border-neutral-200">
            ...
            {acc("delivery", "DELIVERY & STORE PICKUP", (
              <ul className="space-y-1.5">
                ...
                <li>• Store pickup: Sarafa Bazaar, Sarafa Bazaar, Near PNB ATM, Jodhpur. Same-day pickup available.</li>
              </ul>
            ))}
          </div>
        </div>
      ...
      <div className="lg:hidden fixed bottom-[60px] inset-x-0 z-30 px-3 pb-2">
        <div className="bg-white rounded-2xl shadow-[0_10px_40px_-10px_rgba(0,0,0,0.45)] border border-neutral-200 p-2 flex gap-2">
          <button onClick={() => toggleWishlist(product.id)} className={cn("w-12 grid place-items-center rounded-xl border-2 shrink-0", wished ? "border-black bg-black text-[#e8c877]" : "border-neutral-200")} aria-label="Wishlist"><Heart size={19} className={wished ? "fill-[#e8c877]" : ""} /></button>
          <button onClick={handleAdd} disabled={!inStock} className="flex-1 py-3 rounded-xl border-2 border-black font-extrabold text-[12px] disabled:opacity-40">ADD TO CART</button>
          <button onClick={handleBuy} disabled={!inStock} className="flex-1 py-3 rounded-xl bg-black text-[#e8c877] font-extrabold text-[12px] disabled:opacity-40">BUY NOW</button>
        </div>
      </div>
      ...
      <div id="reviews" className="mt-12 grid lg:grid-cols-2 gap-8">
            ...
            {reviews.slice(0, 5).map((r, i) => (
              <div key={i} className="border border-neutral-200 rounded-2xl p-4">
                <div className="flex items-center gap-2">
                  <span className="w-8 h-8 rounded-full bg-black text-[#e8c877] grid place-items-center text-xs font-extrabold">{r.userName[0]}</span>
                  <div>
        ...
        <div className="border border-neutral-200 rounded-3xl p-5 sm:p-7 h-fit bg-neutral-50/50">
          ...
          {reviewDone ? (
            ...
            <div className="space-y-3 mt-4">
              ...
              <div className="flex items-center gap-1.5">
                ...
                {[1, 2, 3, 4, 5].map((i) => (
                  <button key={i} onClick={() => setReviewRating(i)} aria-label={`${i} stars`}><Star size={22} className={i <= reviewRating ? "fill-[#c9a24b] text-[#c9a24b]" : "text-neutral-300"} /></button>
                ))}
              </div>
              ...
              <button
                onClick={async () => {
                  ...
                  await fetch("/api/reviews", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ productId: product.id, userName: reviewName, rating: reviewRating, comment: reviewText }) });
                  setReviewDone(true);
                }}
                className="w-full py-3.5 rounded-xl bg-black text-[#e8c877] font-extrabold text-[13px] tracking-wider"
              >
                SUBMIT REVIEW
              </button>
            </div>
          ...
          <div className="mt-5 flex items-start gap-2 text-[12px] text-neutral-500">
            <MapPin size={15} className="shrink-0 mt-0.5 text-[#9a7420]" />
            <p>Bought at our Sarafa Bazaar store? Mention your experience — it helps Jodhpur shoppers.</p>
          </div>
        </div>
      </div>
          ...
          <div className="bg-white rounded-3xl max-w-lg w-full p-6" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-[13px]">
                <thead><tr className="bg-black text-[#e8c877]"><th className="p-2.5 text-left rounded-l-lg">Size</th><th className="p-2.5">Chest (in)</th><th className="p-2.5">Waist (in)</th><th className="p-2.5 text-right rounded-r-lg">Length</th></tr></thead>
                <tbody>
                  {(product.audience === "kids"
                    ? [["2-3Y", "21", "20", "16"], ["4-5Y", "23", "21", "18"], ["6-7Y", "25", "22", "20"], ["8-9Y", "27", "23", "22"], ["10-11Y", "29", "24", "24"], ["12-13Y", "31", "26", "26"]]
                    : [["XS", "34", "28", "26"], ["S", "36", "30", "27"], ["M", "38", "32", "28"], ["L", "40", "34", "29"], ["XL", "42", "36", "30"], ["XXL", "44", "38", "30.5"], ["3XL", "46", "40", "31"]]
                  ).map((r) => (
            ...
            <div className="flex items-center gap-2 mt-3 text-[12px] font-bold text-[#9a7420]"><ShieldCheck size={15} /> Easy 7-day size exchange at store</div>
          </div>
        </div>
      )}
    </div>
