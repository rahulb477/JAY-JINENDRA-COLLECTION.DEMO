...
export default function QuickView() {
  ...
  useEffect(() => {
    if (!quickViewSlug) {
      const clear = setTimeout(() => { setP(null); setSize(""); }, 0);
              ...
              <div className="grid sm:grid-cols-2">
                <div className="relative aspect-[3/4] bg-neutral-100 sm:rounded-l-3xl overflow-hidden">
                  ...
                  {discountPct(p.mrp, p.price) > 0 && (
                    <span className="absolute top-3 left-3 text-[11px] font-bold px-2.5 py-1 rounded-full bg-[#c9a24b] text-black">{discountPct(p.mrp, p.price)}% OFF</span>
                  )}
                </div>
                <div className="p-6 sm:p-8 flex flex-col">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {(p.sizes?.length ? p.sizes : SIZES_MEN.slice(1, 6)).map((s) => (
                      <button key={s} onClick={() => setSize(s)} className={`min-w-[46px] h-10 px-3 rounded-lg border text-sm font-bold transition ${size === s ? "border-black bg-black text-[#e8c877]" : "border-neutral-200 hover:border-black"}`}>
                        {s}
                      </button>
                    ))}
                  </div>
                  <div className="flex gap-2 mt-6">
                    <button
                      ...
                      className={`w-12 h-12 shrink-0 grid place-items-center rounded-xl border-2 ${isWishlisted(p.id) ? "border-black bg-black text-[#e8c877]" : "border-neutral-200"}`}
                      aria-label="Wishlist"
                    >
                      <Heart size={19} className={isWishlisted(p.id) ? "fill-[#e8c877]" : ""} />
                    </button>
                    <button
                      onClick={() => {
                      ...
                      className="flex-1 h-12 rounded-xl bg-black text-[#e8c877] font-bold text-sm tracking-wide flex items-center justify-center gap-2"
                    >
                      <ShoppingBag size={18} /> ADD TO BAG
                    </button>
                  </div>
                  <Link href={`/product/${p.slug}`} onClick={() => setQuickViewSlug(null)} className="mt-3 text-center text-[13px] font-bold text-[#9a7420] underline underline-offset-4">
                    View full details
                  </Link>
                </div>
              </div>
