...
export default function ShopClient({
  ...
  useEffect(() => {
    ...
    if (sizes.length === 1) params.set("size", sizes[0]);
    if (fits.length === 1) params.set("fit", fits[0]);
    if (colors.length === 1) params.set("color", colors[0]);
    ...
    const t = setTimeout(async () => {
      try {
        ...
        if (fits.length > 1) list = list.filter((p) => p.fits?.some((f) => fits.includes(f)));
        if (colors.length > 1) list = list.filter((p) => p.colors?.some((c) => colors.some((x) => c.toLowerCase().includes(x.toLowerCase()))));
        setProducts(list);
      } catch {
        setProducts([]);
      } finally {
  ...
  const sizeOptions = useMemo(() => {
    if (audience === "kids") return SIZES_KIDS;
    if (audience === "men") return [...SIZES_MEN, "28", "30", "32", "34", "36"];
    return [...SIZES_MEN, "28", "30", "32", "34", "36", ...SIZES_KIDS];
  }, [audience]);
    ...
    <div className="space-y-6">
      {!fixedCategory && (
        <div>
          ...
          <div className="space-y-1">
            <button onClick={() => setCategory("")} className={`w-full text-left px-3 py-2 rounded-lg text-sm font-semibold ${!category ? "bg-black text-[#e8c877]" : "hover:bg-neutral-100"}`}>All</button>
            {cats.map((c) => (
              <button key={c.slug} onClick={() => setCategory(category === c.slug ? "" : c.slug)} className={`w-full text-left px-3 py-2 rounded-lg text-sm font-semibold ${category === c.slug ? "bg-black text-[#e8c877]" : "hover:bg-neutral-100"}`}>{c.name}</button>
            ))}
          </div>
        </div>
      )}
      <div>
        ...
        <div className="flex flex-wrap gap-2">
          {sizeOptions.map((s) => (
            <button key={s} onClick={() => toggle(sizes, s, setSizes)} className={`min-w-[44px] h-9 px-2.5 rounded-lg border text-[13px] font-bold ${sizes.includes(s) ? "border-black bg-black text-[#e8c877]" : "border-neutral-200"}`}>{s}</button>
          ))}
        </div>
      </div>
      <div>
  ...
  return (
    <div className="max-w-[1400px] mx-auto px-4 sm:px-6 py-6 sm:py-10 pb-36 md:pb-10">
      {/* Breadcrumb */}
          ...
          <div className="relative">
            ...
            {sortOpen && (
              <div className="absolute right-0 top-12 w-56 bg-white border border-neutral-200 rounded-2xl shadow-xl overflow-hidden z-20">
                {SORTS.map((s) => (
                  <button key={s.v} onClick={() => { setSort(s.v); setSortOpen(false); }} className={`w-full text-left px-4 py-3 text-sm font-semibold hover:bg-neutral-50 ${sort === s.v ? "bg-neutral-900 text-[#e8c877]" : ""}`}>{s.l}</button>
                ))}
              </div>
            )}
          </div>
        ...
        <div className="flex-1 min-w-0">
          ...
          {loading ? (
            ...
            <div className="text-center py-20">
              ...
              <button onClick={() => { setSizes([]); setFits([]); setColors([]); setCategory(""); setSearch(""); setMinDiscount(0); setMaxPrice(2500); }} className="mt-5 px-8 py-3 rounded-xl bg-black text-[#e8c877] text-sm font-bold">CLEAR FILTERS</button>
            </div>
          ) : (
      ...
      <div className="md:hidden fixed bottom-[60px] inset-x-0 z-30 px-4 pb-3">
        <div className="grid grid-cols-2 gap-2 bg-white rounded-2xl shadow-[0_10px_40px_-10px_rgba(0,0,0,0.4)] border border-neutral-200 p-2">
          <button onClick={() => setFiltersOpen(true)} className="flex items-center justify-center gap-2 py-3 rounded-xl bg-black text-white text-[13px] font-extrabold tracking-wide">
            <SlidersHorizontal size={16} /> FILTER {activeCount > 0 && <span className="w-5 h-5 rounded-full bg-[#c9a24b] text-black text-[10px] grid place-items-center">{activeCount}</span>}
          </button>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[70] bg-black/60" onClick={() => setFiltersOpen(false)}>
            <motion.div initial={{ y: "100%" }} animate={{ y: 0 }} exit={{ y: "100%" }} transition={{ type: "spring", damping: 30, stiffness: 300 }} className="absolute bottom-0 inset-x-0 max-h-[88vh] bg-white rounded-t-3xl flex flex-col" onClick={(e) => e.stopPropagation()}>
              <div className="flex items-center justify-between p-4 border-b border-neutral-100">
                <p className="font-extrabold tracking-wide">FILTERS {activeCount > 0 && <span className="text-[#9a7420]">({activeCount})</span>}</p>
              ...
              <div className="p-4 border-t border-neutral-100">
                <button onClick={() => setFiltersOpen(false)} className="w-full py-4 rounded-xl bg-black text-[#e8c877] font-extrabold text-sm tracking-wider">SHOW {products.length} STYLES</button>
              </div>
            </motion.div>
          </motion.div>
        )}
        {sortOpen && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="md:hidden fixed inset-0 z-[70] bg-black/60" onClick={() => setSortOpen(false)}>
            <motion.div initial={{ y: "100%" }} animate={{ y: 0 }} exit={{ y: "100%" }} className="absolute bottom-0 inset-x-0 bg-white rounded-t-3xl p-4" onClick={(e) => e.stopPropagation()}>
              ...
              {SORTS.map((s) => (
                <button key={s.v} onClick={() => { setSort(s.v); setSortOpen(false); }} className={`w-full text-left px-4 py-3.5 rounded-xl text-sm font-bold ${sort === s.v ? "bg-black text-[#e8c877]" : "hover:bg-neutral-100"}`}>{s.l}</button>
              ))}
            </motion.div>
          </motion.div>
        )}
