...
const badgeStyle: Record<string, string> = {
  ...
  TRENDING: "bg-black text-[#e8c877] border border-[#c9a24b]",
  BESTSELLER: "bg-[#c9a24b] text-black",
    ...
    <span className="inline-flex items-center gap-0.5" aria-label={`Rated ${r} out of 5`}>
      {[1, 2, 3, 4, 5].map((i) => (
        <Star
          key={i}
          size={size}
          className={i <= Math.round(r) ? "fill-[#c9a24b] text-[#c9a24b]" : "fill-neutral-200 text-neutral-200"}
        />
      ))}
    </span>
  );
  ...
  return (
    ...
    >
      <div className={cn("relative overflow-hidden bg-neutral-100", large ? "aspect-[3/4]" : "aspect-[3/4]")}>
        <Link href={`/product/${p.slug}`} aria-label={p.name}>
          {img1 ? (
            ...
            <div className="absolute inset-0 grid place-items-center bg-gradient-to-br from-neutral-900 to-neutral-700 text-[#e8c877] font-display text-lg p-4 text-center">
              {p.name}
            </div>
          )}
        </Link>
        ...
        <button
          ...
          className={cn(
            ...
            wished ? "bg-black text-[#e8c877]" : "bg-white/95 text-neutral-700 hover:text-black"
          )}
        >
          <Heart size={17} className={wished ? "fill-[#e8c877]" : ""} />
        </button>
        ...
        <div className="absolute inset-x-2 bottom-2 flex gap-2 opacity-100 sm:opacity-0 sm:translate-y-2 sm:group-hover:opacity-100 sm:group-hover:translate-y-0 transition-all duration-300">
          ...
          >
            <Eye size={15} /> <span className="hidden min-[420px]:inline">QUICK </span>VIEW
          </button>
          <button
            onClick={() =>
              addToCart({
            ...
            className="flex-1 h-10 rounded-xl bg-black text-[#e8c877] text-[12px] font-bold tracking-wide flex items-center justify-center gap-1.5 hover:bg-neutral-900 shadow"
      ...
      <div className="p-3 flex flex-col gap-1 flex-1">
        <p className="text-[10px] font-semibold tracking-[0.14em] text-[#9a7420] uppercase">{p.categorySlug.replace("men-", "").replace("kids-", "").replace(/-/g, " ")}</p>
        <Link href={`/product/${p.slug}`} className="font-semibold text-[13.5px] leading-snug clamp-2 hover:text-[#9a7420] transition">
          {p.name}
        </Link>
