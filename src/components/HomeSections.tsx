...
export function Hero({ banners }: { banners: Banner[] }) {
  ...
  const slides = banners.length ? banners : [{ id: 0, title: "JAI JINENDRA COLLECTION.", subtitle: "Men's Fashion. Wholesale & Retail. Delivery across India — Sarafa Bazaar, Jodhpur.", image: "/images/jjc-hero.jpg", ctaText: "SHOP COLLECTION", ctaLink: "/shop" }];
  useEffect(() => {
  ...
  return (
    <section className="relative bg-black overflow-hidden hero-grain" aria-label="Featured">
      ...
      <div className="relative max-w-[1400px] mx-auto px-4 sm:px-8 min-h-[480px] sm:min-h-[560px] lg:min-h-[600px] flex items-center py-16">
        <AnimatePresence mode="wait">
          <motion.div key={idx} initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} transition={{ duration: 0.55 }} className="max-w-2xl">
            <p className="inline-flex items-center gap-2 text-[11px] sm:text-xs font-bold tracking-[0.3em] text-[#e8c877] border border-[#c9a24b]/50 rounded-full px-4 py-1.5 bg-black/40 backdrop-blur">
              <Flame size={13} /> MEN&apos;S FASHION · WHOLESALE &amp; RETAIL
            </p>
            <h1 className="font-display font-black text-white text-[34px] min-[390px]:text-[42px] sm:text-6xl lg:text-7xl leading-[1.02] mt-5">
              {s.title.split(".").filter(Boolean).map((part, i) => (
                <span key={i} className="block">{part.trim()}{i === 0 ? <span className="brand-text">.</span> : ""}</span>
              ))}
            </h1>
            <p className="text-neutral-300 text-[15px] sm:text-lg mt-4 max-w-xl">{s.subtitle}</p>
            <div className="flex flex-wrap gap-3 mt-8">
              <Link href={s.ctaLink || "/men"} className="px-6 sm:px-8 py-4 rounded-xl brand-bg text-black font-extrabold text-[13px] tracking-[0.12em] hover:brightness-110 transition shadow-[0_10px_40px_-10px_rgba(201,162,75,0.6)]">
                {s.ctaText || "SHOP COLLECTION"}
              </Link>
              <a href={STORE.whatsapp} target="_blank" rel="noreferrer" className="px-6 sm:px-8 py-4 rounded-xl border border-white/30 text-white font-extrabold text-[13px] tracking-[0.12em] hover:bg-white hover:text-black transition backdrop-blur">
                ORDER ON WHATSAPP
              </a>
              <Link href="/store" className="px-2 py-4 text-[13px] font-extrabold tracking-[0.12em] text-[#e8c877] hover:text-white transition">
                VISIT STORE
              </Link>
            </div>
            <div className="flex flex-wrap items-center gap-x-5 gap-y-2 mt-8 text-[12px] text-neutral-400 font-medium">
              <span className="flex items-center gap-1.5"><BadgeCheck size={15} className="text-[#e8c877]" /> Wholesale &amp; retail</span>
              <span className="flex items-center gap-1.5"><Truck size={15} className="text-[#e8c877]" /> Delivery across India</span>
              <span className="flex items-center gap-1.5"><StoreIcon size={15} className="text-[#e8c877]" /> Sarafa Bazaar, Jodhpur</span>
            </div>
          </motion.div>
        </AnimatePresence>
      </div>
      ...
      <div className="absolute bottom-5 left-1/2 -translate-x-1/2 flex items-center gap-2">
        {slides.map((_, i) => (
          <button key={i} onClick={() => setIdx(i)} aria-label={`Slide ${i + 1}`} className={`h-1.5 rounded-full transition-all ${i === idx % slides.length ? "w-8 bg-[#e8c877]" : "w-1.5 bg-white/40"}`} />
        ))}
      </div>
      {slides.length > 1 && (
        <>
          <button onClick={() => setIdx((idx - 1 + slides.length) % slides.length)} aria-label="Previous" className="hidden sm:grid absolute left-4 top-1/2 -translate-y-1/2 w-11 h-11 rounded-full bg-white/10 backdrop-blur text-white place-items-center hover:bg-[#c9a24b] hover:text-black transition"><ChevronLeft size={22} /></button>
          <button onClick={() => setIdx((idx + 1) % slides.length)} aria-label="Next" className="hidden sm:grid absolute right-4 top-1/2 -translate-y-1/2 w-11 h-11 rounded-full bg-white/10 backdrop-blur text-white place-items-center hover:bg-[#c9a24b] hover:text-black transition"><ChevronRight size={22} /></button>
        </>
      )}
    </section>
  );
...
export function Marquee() {
  const items = ["WHOLESALE & RETAIL", "DELIVERY ACROSS INDIA", "NEW DROPS EVERY WEEK", "EASY STORE PICKUP", "COD AVAILABLE", "SARAFA BAZAAR · JODHPUR"];
  return (
    <div className="bg-[#c9a24b] overflow-hidden py-2.5" aria-hidden>
      <div className="flex w-max animate-marquee gap-0">
        {[0, 1].map((k) => (
          <div key={k} className="flex shrink-0">
            {items.map((t, i) => (
              <span key={i} className="flex items-center gap-3 px-6 text-[12px] font-extrabold tracking-[0.2em] text-black whitespace-nowrap">
                <Star size={12} className="fill-black" /> {t}
              </span>
            ))}
          </div>
        ))}
  ...
  const Card = ({ c, big = false }: { c: Cat; big?: boolean }) => (
    <Link href={c.parentSlug ? `/shop?category=${c.slug}` : `/${c.slug}`} className={`group relative rounded-2xl overflow-hidden img-zoom shrink-0 ${big ? "w-[240px] sm:w-[300px] aspect-[4/5]" : "w-[150px] sm:w-[180px] aspect-[3/4]"}`}>
      ...
      <div className="absolute bottom-0 inset-x-0 p-3 sm:p-4">
        ...
        <p className="text-[#e8c877] text-[11px] font-semibold flex items-center gap-1 mt-0.5">SHOP NOW <ArrowRight size={12} className="group-hover:translate-x-1 transition" /></p>
      </div>
    </Link>
  );
  return (
    <section className="max-w-[1400px] mx-auto px-4 sm:px-6 py-10 sm:py-14">
      <Reveal>
        <div className="flex items-end justify-between mb-5">
          <div>
            <p className="text-[11px] font-bold tracking-[0.25em] text-[#9a7420]">START HERE</p>
            <h2 className="font-display font-extrabold text-2xl sm:text-4xl mt-1">Shop by Category</h2>
          </div>
          <Link href="/shop" className="hidden sm:flex items-center gap-1.5 text-sm font-bold hover:text-[#9a7420]">VIEW ALL <ArrowRight size={16} /></Link>
        </div>
      </Reveal>
      <Reveal delay={100}>
          ...
          {[categories.find((c) => c.slug === "men"), categories.find((c) => c.slug === "kids")].filter(Boolean).map((c) => (
            <Link key={c!.slug} href={`/${c!.slug}`} className="group relative rounded-2xl overflow-hidden img-zoom aspect-[16/10] sm:aspect-[21/9]">
              ...
              <div className="absolute bottom-0 p-4 sm:p-7">
                <p className="text-[#e8c877] text-[10px] sm:text-xs font-bold tracking-[0.3em]">{c!.slug === "men" ? "PRIMARY COLLECTION" : "LITTLE STARS"}</p>
                <p className="font-display font-black text-white text-3xl sm:text-5xl mt-1">{c!.name.toUpperCase()}</p>
                <span className="inline-flex items-center gap-2 mt-2 sm:mt-3 px-4 sm:px-6 py-2 sm:py-2.5 rounded-lg bg-white text-black text-[11px] sm:text-xs font-extrabold tracking-wider group-hover:bg-[#c9a24b] transition">EXPLORE <ArrowRight size={14} /></span>
              </div>
            </Link>
          ))}
        </div>
      ...
      <div className="max-w-[1400px] mx-auto px-4 sm:px-6">
        <Reveal>
          <div className="flex items-end justify-between mb-6">
            <div>
              {dark ? <p className="text-[11px] font-bold tracking-[0.25em] text-[#e8c877] flex items-center gap-1.5"><Flame size={13} /> MOST WANTED</p>
                : <p className="text-[11px] font-bold tracking-[0.25em] text-[#9a7420]">FRESH DROPS</p>}
            ...
            <div className="flex items-center gap-2">
              <Link href={link} className={`hidden sm:inline-flex items-center gap-1.5 text-sm font-bold mr-2 ${dark ? "text-[#e8c877]" : "hover:text-[#9a7420]"}`}>{linkLabel} <ArrowRight size={16} /></Link>
              <button onClick={() => scroll(-1)} aria-label="Scroll left" className={`w-10 h-10 rounded-full grid place-items-center border transition ${dark ? "border-white/20 text-white hover:bg-[#c9a24b] hover:text-black" : "border-neutral-200 hover:bg-black hover:text-white"}`}><ChevronLeft size={19} /></button>
              <button onClick={() => scroll(1)} aria-label="Scroll right" className={`w-10 h-10 rounded-full grid place-items-center border transition ${dark ? "border-white/20 text-white hover:bg-[#c9a24b] hover:text-black" : "border-neutral-200 hover:bg-black hover:text-white"}`}><ChevronRight size={19} /></button>
            </div>
          </div>
        </Reveal>
        ...
        <div className="sm:hidden mt-4 text-center">
          <Link href={link} className={`inline-flex items-center gap-1.5 text-[13px] font-extrabold tracking-wider px-6 py-3 rounded-xl ${dark ? "bg-[#c9a24b] text-black" : "bg-black text-[#e8c877]"}`}>{linkLabel} <ArrowRight size={15} /></Link>
        </div>
      </div>
    </section>
  );
...
export function OffersSection({ offers }: { offers: Offer[] }) {
  ...
  return (
    <section className="bg-[#f5f5f5] py-12 sm:py-16 border-y border-[#e4e4e4]">
      <div className="max-w-[1400px] mx-auto px-4 sm:px-6">
        <Reveal>
          <div className="text-center mb-8">
            <p className="text-[11px] font-bold tracking-[0.3em] text-[#9a7420] flex items-center justify-center gap-1.5"><Tag size={13} /> SPECIAL OFFERS</p>
          ...
          {offers.map((o, i) => (
            <Reveal key={o.id} delay={i * 100}>
              <Link href={o.ctaLink || "/offers"} className="group relative block rounded-3xl overflow-hidden aspect-[4/5] sm:aspect-[3/4] img-zoom">
                ...
                {o.badge && <span className="absolute top-4 left-4 text-[10px] font-extrabold tracking-[0.15em] px-3 py-1.5 rounded-full bg-[#c9a24b] text-black">{o.badge}</span>}
                <div className="absolute bottom-0 inset-x-0 p-5 sm:p-6">
                  <p className="text-[#e8c877] text-[11px] font-bold tracking-[0.25em]">{o.subtitle}</p>
                  ...
                  <span className="inline-flex items-center gap-2 mt-4 px-6 py-3 rounded-xl bg-white text-black text-[12px] font-extrabold tracking-wider group-hover:bg-[#c9a24b] transition">
  ...
  return (
    <section className="max-w-[1400px] mx-auto px-4 sm:px-6 py-12 sm:py-16">
      <Reveal>
        <div className="flex items-end justify-between mb-6">
          <div>
            <p className="text-[11px] font-bold tracking-[0.25em] text-[#9a7420]">CURATED FOR YOU</p>
            <h2 className="font-display font-extrabold text-2xl sm:text-4xl mt-1">Shop by Category</h2>
          </div>
        </div>
      </Reveal>
          ...
          <Reveal key={c.slug} delay={(i % 4) * 80}>
            <Link href={`/shop?category=${c.slug}`} className="group relative block rounded-2xl overflow-hidden aspect-square img-zoom">
              ...
              <div className="absolute bottom-0 inset-x-0 p-4 flex items-center justify-between">
                ...
                <span className="w-9 h-9 rounded-full bg-white/15 backdrop-blur grid place-items-center text-white group-hover:bg-[#c9a24b] group-hover:text-black transition"><ArrowRight size={16} /></span>
              </div>
            </Link>
          </Reveal>
        ))}
    ...
    <section className="bg-black py-12 sm:py-16 overflow-hidden">
      <div className="max-w-[1400px] mx-auto px-4 sm:px-6">
        <Reveal>
          <div className="flex items-end justify-between mb-6">
            <div>
              <p className="text-[11px] font-bold tracking-[0.25em] text-[#e8c877]">FOLLOW THE HYPE</p>
              <h2 className="font-display font-extrabold text-2xl sm:text-4xl mt-1 text-white">As Seen on Instagram</h2>
              <p className="text-sm text-neutral-400 mt-1">@jai_jinendra_collection__ · Reels, drops & store videos</p>
            </div>
            <a href={STORE.instagram} target="_blank" rel="noreferrer" className="hidden sm:inline-flex items-center gap-2 px-6 py-3 rounded-xl brand-bg text-black text-[12px] font-extrabold tracking-wider">FOLLOW US <ArrowRight size={15} /></a>
          </div>
        </Reveal>
            ...
            <Reveal key={r.id} delay={i * 60}>
              <div className="relative w-[168px] sm:w-[210px] aspect-[9/16] rounded-2xl overflow-hidden shrink-0 group border border-white/10">
                ...
                <div className="absolute bottom-0 inset-x-0 p-3">
                  ...
                  {r.productSlug ? (
                    <Link href={`/product/${r.productSlug}`} className="mt-2 flex items-center justify-center gap-1 w-full py-2 rounded-lg bg-white text-black text-[10.5px] font-extrabold tracking-wider hover:bg-[#c9a24b] transition">SHOP THIS LOOK</Link>
                  ) : r.instagramUrl ? (
...
export function WhyShop() {
  const items = [
    ...
    { icon: <StoreIcon size={26} />, t: "LOCAL STORE PICKUP", s: "Sarafa Bazaar · easy exchange" },
  ];
  return (
    <section className="max-w-[1400px] mx-auto px-4 sm:px-6 py-12 sm:py-16">
      <Reveal>
        <h2 className="font-display font-extrabold text-2xl sm:text-4xl text-center">Why Shop JJC</h2>
      </Reveal>
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 mt-7">
        {items.map((x, i) => (
          <Reveal key={x.t} delay={i * 80}>
            <div className="h-full rounded-2xl border border-neutral-200 bg-white p-5 sm:p-7 text-center hover:border-[#c9a24b] hover:shadow-[0_16px_40px_-20px_rgba(201,162,75,0.5)] transition">
              <span className="inline-grid place-items-center w-13 h-13 sm:w-14 sm:h-14 rounded-full bg-black text-[#e8c877] p-3.5">{x.icon}</span>
...
export function StoreLocation() {
  return (
    <section className="bg-[#f5f5f5] border-t border-[#e4e4e4]">
      <div className="max-w-[1400px] mx-auto px-4 sm:px-6 py-12 sm:py-16 grid lg:grid-cols-2 gap-8 items-center">
        <Reveal>
          <div>
            <p className="text-[11px] font-bold tracking-[0.3em] text-[#9a7420] flex items-center gap-1.5"><MapPin size={13} /> VISIT OUR STORE</p>
            <h2 className="font-display font-black text-3xl sm:text-5xl mt-2 leading-tight">JAI JINENDRA<br /><span className="text-[#c9a24b]">COLLECTION</span></h2>
            <p className="text-[11px] font-bold tracking-[0.3em] text-neutral-500 mt-2">JJC · WHOLESALE &amp; RETAIL · DELIVERY ACROSS INDIA</p>
            ...
            <div className="grid grid-cols-2 gap-2.5 mt-6 max-w-md">
              <Link href="/store" className="flex items-center justify-center gap-2 py-3.5 rounded-xl bg-black text-[#e8c877] text-[12px] font-extrabold tracking-wider"><StoreIcon size={15} /> VISIT STORE</Link>
              ...
              <a href={`tel:${STORE.phoneRaw}`} className="flex items-center justify-center gap-2 py-3.5 rounded-xl brand-bg text-black text-[12px] font-extrabold tracking-wider"><Phone size={15} /> CALL NOW</a>
              <a href={`mailto:${STORE.email}`} className="flex items-center justify-center gap-2 py-3.5 rounded-xl border-2 border-black text-[12px] font-extrabold tracking-wider">EMAIL JJC</a>
            </div>
          </div>
        </Reveal>
        <Reveal delay={120}>
          <div className="relative rounded-3xl overflow-hidden border border-[#e4e4e4] bg-white shadow-xl">
            <iframe
              title="Jai Jinendra Collection store map"
              src="https://www.google.com/maps?q=Sarafa+Bazaar+Jodhpur+Rajasthan&output=embed"
            ...
            <div className="absolute bottom-4 left-4 right-4 sm:right-auto bg-black/90 backdrop-blur rounded-2xl p-4 flex items-center gap-3">
              <span className="w-10 h-10 rounded-full brand-bg grid place-items-center shrink-0"><MapPin size={18} className="text-black" /></span>
              <div>
                <p className="text-white font-bold text-[13px]">Sarafa Bazaar</p>
                <p className="text-neutral-400 text-[12px]">Near PNB ATM · Jodhpur</p>
              </div>
            </div>
          </div>
        </Reveal>
